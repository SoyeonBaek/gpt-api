...
async def handle_calender_insert(title, starttime, endtime):
...
async def handle_gpt_function(text: str):
    ...
    if hasattr(choice, "message") and choice.message.tool_calls:
        for call in choice.message.tool_calls:
            ...
            if name == "handle_chatbot":
                await handle_chatbot(**args)
            elif name == "handle_image":
                await handle_image(**args)
            elif name == "handle_tts":
                await handle_tts(**args)
            elif name == "handle_calender_insert":
                await handle_calender_insert(**args)
